...
public class ScanPackages {
  ...
  private static String PATH_FOR_SCAN = "lab8.classes";
    ...
    for (Class<?> cl : classList) {
      ...
      if (cl.getSimpleName().contains(className)) {
        if (className.equals("Users")) {
          ...
          Field[] usersFields = Users.class.getDeclaredFields();
          fields = new Field[usersFields.length + personFields.length];
          Arrays.setAll(fields, i ->
                  (i < personFields.length ? personFields[i] : usersFields[i - personFields.length]));
